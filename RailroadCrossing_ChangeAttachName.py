...
fields = ['rel_globalid', 'DATA', 'ATT_NAME']
...
# Process: Copy the attachments and Change their name afterwards
tracking = [1 for _ in folders]  # tracks the file name in each folder
try:
    with arcpy.da.UpdateCursor(inTable, fields) as cursor:
        for row in cursor:
            if row[0] in ID:
